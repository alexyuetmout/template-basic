...
interface ManualOneTapProps {
  callbackURL?: string;
  className?: string;
}

export function ManualOneTap({ callbackURL = "/dashboard", className }: ManualOneTapProps) {
  ...
  return (
    <Button
      ...
      disabled={isLoading}
      className={`w-full h-12 border-neutral-300 dark:border-neutral-600 ${className}`}
    >
      ...
      {isLoading ? "Loading One Tap..." : "Google One Tap"}
    </Button>
  );
}